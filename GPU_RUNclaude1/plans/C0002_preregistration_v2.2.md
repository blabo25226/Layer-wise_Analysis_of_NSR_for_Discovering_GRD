# C0002 — Preregistration **v2.2** (DRAFT — NOT FROZEN)

| field | value |
|---|---|
| cycle | `C0002` |
| stage | 3 (design, revision 2.2) |
| status | **`DRAFT_PENDING_SUPERVISOR_CHECK`** — this document freezes nothing. It becomes binding only when the supervisor has checked that the send-back of 2026-09-12 is faithfully reflected and marks it `FROZEN`. |
| supersedes | `plans/C0002_preregistration_v2.1.md` + `.json`, status `DRAFT_PENDING_SUPERVISOR_CHECK`, which superseded v2, which superseded v1 |
| v1 / v2 / v2.1 disposition | **v1, v2 and v2.1 are retained unedited as the historical record.** None is amended, annotated or deleted. v1 is the document the two reviewers reviewed; v2 is the document on which the supervisor's three decisions were taken; **v2.1 is the document the supervisor verified and partially sent back.** |
| author role | `lansr-research-methodologist` (drafting only). The author of v1, v2 and v2.1 is the author of v2.2; **the two reviewers were independent of the author and of each other.** |
...
| **what v2.2 changes** | **exactly one thing: §13.4 rule 2.** Cross-context agreement is required of **P2** and **not of P1**, because the contract's own §13.2 branch 1 already says `C_raw`'s P1 is **not a search-error claim** — so requiring P1 to agree across contexts required it to agree with a quantity that measures something else. Everything else in v2.1 is carried forward, including the two push-backs and the one flagged interpretation the supervisor confirmed. **§26** is the record. |
| thresholds moved in v2.1 and v2.2 | **NONE.** Not one bar, cap, tolerance, cutoff, window, floor, partner count, seed or ceiling differs from v2. §25.4 is the clause-by-clause table, **independently verified by the supervisor: 31 rows, zero rows whose v2 value differs.** v2.2 moves none of them either — it corrects **which quantities are commensurable**, nothing else. |
...
6. **NEW IN v2.2 — P1 and P2 are not commensurable across contexts in the same way, and the
   cycle-level rule no longer pretends they are.** v2.1's §13.4 rule 2 made a cross-context
   disagreement collapse to `conditioning_sensitive` for **both** certificates. The supervisor sent
   that back on 2026-09-12 (`plans/C0002_supervisor_decisions.md`, 追補), on grounds internal to this
   contract:
   ...
   **Frozen consequence (§13.4):** rule 2 applies to **P2**, where `me_best` carries no search
   semantics and the two contexts **are** commensurable. **For P1 the `C_gen` result stands on its
   own**, with the `C_raw` downgraded value and the difference between them **reported alongside,
   mandatorily**. Rule 3 — one context `undecidable` — **stays as written for P1 too**, because that
   is an instrument-failure question, not a commensurability question.
...
**NEW in v2.2 — for P1 there is a SECOND and INDEPENDENT reason, and it is the stronger one.** The
non-independence above is about *shared inputs*. For P1 the objection is prior to that: **the two
contexts' P1 values are not the same quantity.** `C_gen`'s P1 is a certified search-error rate;
`C_raw`'s P1 is "the truth outscores all 50 candidates under a conditioning the search did not see",
which §13.2 branch 1 and §8.7 both state is **not** a search-error claim. A number that is not a
measurement of X can neither confirm nor disconfirm a measurement of X. So for P1 the `C_raw` value:

- **may not be read as confirmation** when it agrees (non-independence, and non-commensurability);
- **may not be read as disconfirmation, a failed check, a robustness failure or a reason to doubt**
  when it disagrees (non-commensurability). §8.7 forbids both readings by name.
...
- **Forbidden**: presenting `LB(P1 | C_raw)` as confirming, corroborating, validating or "checking"
  ...
  doubt on it.** Two independent reasons: the contexts are not independent, **and** the two values are
  not the same quantity (§7.3.2 condition 4). A number that is not a measurement of X neither confirms
  nor disconfirms a measurement of X.
...
proposition.** The exposure v2.1's conjunction controlled for P1 was an artefact of treating two
**non-commensurable** quantities as two shots at one proposition. Once `C_raw`'s P1 is not a claim
about search error, there is **one** test of "a search error is certified on the majority of systems",
evaluated **once**, in **one** context, at a one-sided error rate of **0.05**. Removing the
conjunction does not uncontrol an exposure; it removes a control on a non-exposure.
...
- **What replaces it is external, not internal**: §19's replication gate, which already fires
  **mandatorily** on a certified lower bound reported as prevalent and requires a **genuinely
  independent** factor — a fresh process and artifact path, a different `CONTROL_CELL_SEED`, or an
  independently written scorer. **§19 is now P1's only cross-check and may not be waived.**
...
#### Rule 0 — the commensurability premise, stated before the rules that depend on it

| certificate | commensurable across contexts? | why |
|---|---|---|
| **P2** (`me_best`, Certificate N) | **YES** | `me_best = 1[lp_gt_lse < lp_best]` certifies that the truth is not the model's argmax **under the conditioning it was scored in**. That proposition is the same proposition in both contexts; only the conditioning differs. §13.2 branch 3 records that this branch's meaning does **not** downgrade across contexts. Two measurements of one quantity may be required to agree |
| **P1** (`sb_best`, Certificate S) | **NO** | §13.2 branch 1, frozen in v2.1 and derived from F-g in v1, restricts `C_raw`'s P1 to "the truth outscores all 50 candidates under a conditioning the search did not see", which is **explicitly not a search-error claim** (§8.7 forbids Certificate-S language there). `C_gen`'s P1 **is** a certified search-error rate. **These are two different quantities**, and requiring one to agree with the other is a category error, not a robustness check |
...
**and** — the stronger reason for P1 — the two values are **not the same quantity**. A number that is
not a measurement of X neither confirms nor disconfirms a measurement of X. §8.7 forbids both
readings by name.
...
rule 2's ground was defective. Rule 2 asked for **agreement between quantities**; that was a
commensurability question, and P1 fails commensurability. **Rule 3 is an instrument-failure question,
and commensurability has nothing to do with it.** An `undecidable` in either context means a control,
...
robustness evidence. **For P1, §7.3.2's second reason additionally applies**: the two values are not
the same quantity, so neither agreement nor disagreement between them adjudicates anything.
...
**NEW in v2.2 — §19 is P1's ONLY cross-check, and it may not be waived.** Under §13.4 rule 1 the
cycle-level P1 verdict rests on a single context, so it has **no internal cross-check at all**
(§12.2 step 4). The gate already fires **mandatorily** on "either certified lower bound reported as
prevalent", which includes every fired P1. **That mandatory firing is now load-bearing**: it is the
external check that replaced the incidental conservatism v2.1's cross-context conjunction had given
P1, and **no threshold was moved to compensate instead**. A fired P1 that is not taken through §19 is
a contract violation.
...
factors above. **For P1 there is a second reason**: the `C_raw` value is not a measurement of the same
quantity (§13.4 rule 0), so it could not be a replication even if the inputs were independent. **`conditioning_sensitive` additionally fires this gate**, and so does a cycle verdict
...
mandatorily**, because `C_raw`'s P1 is not a search-error claim and the two are not commensurable;
...
value differs.**
**v2.2 moves none of them either.** It changes **which quantities are commensurable** — and therefore
which of them may be required to agree — and nothing else. `LB(P1) > 0.5`, `LB(P2) > 0.5` and every
control bar stand exactly as frozen. §26 records this.
...
- and **§19's replication gate may not be waived on a fired P1** — it is that read's only cross-check
  (§12.2 step 4). **If they agree, that agreement may
...
**P2 is unaffected** because `me_best` carries no search semantics: "the truth is not the model's
argmax under the conditioning it was scored in" is the **same proposition** in both contexts, so the
two measurements **are** commensurable and may be required to agree.
...
3. **The direction is against the campaign's preference, not with it.** P1 **refutes** the
   ...
   conjuncts**. §12.2 step 6 states this check explicitly.
