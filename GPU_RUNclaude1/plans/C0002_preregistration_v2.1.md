# C0002 — Preregistration **v2.1** (DRAFT — NOT FROZEN)

| field | value |
|---|---|
| cycle | `C0002` |
| stage | 3 (design, revision 2.1) |
| status | **`DRAFT_PENDING_SUPERVISOR_CHECK`** — this document freezes nothing. It becomes binding only when the supervisor has checked that the three decisions of `plans/C0002_supervisor_decisions.md` are faithfully reflected and marks it `FROZEN`. |
| supersedes | `plans/C0002_preregistration_v2.md` + `.json`, status `DRAFT_PENDING_SUPERVISOR_CHECK`, which in turn superseded `plans/C0002_preregistration_v1.md` + `.json` |
| v1 / v2 disposition | **v1 and v2 are retained unedited as the historical record.** Neither is amended, annotated or deleted. Every line of v1 stands as the document the two reviewers reviewed; every line of v2 stands as the document on which the supervisor's three decisions were taken. |
| author role | `lansr-research-methodologist` (drafting only). The author of v1 and v2 is the author of v2.1; **the two reviewers were independent of the author and of each other.** |
...
| supervisor decisions implemented | **`plans/C0002_supervisor_decisions.md`, dated 2026-09-12**, three decisions, taken **before** Gate 4 runs and therefore not a post-hoc rescue. What changed relative to v2, and why, is in **§25**. |
| thresholds moved in v2.1 | **NONE.** Not one bar, cap, tolerance, cutoff or seed differs from v2. Every decision below was taken without moving one, and §25 records that clause by clause. |
...
| **BA4** | the generation conditioning is not the scoring conditioning (F-g), **direction unknown** | S1 — **the cross-context comparison of the two co-primary reads** (§8.4). Reported at two resolutions: (i) whether the two contexts fire the **same §13.2 branch**, and (ii) the paired per-system agreement rate of `E3_system` and of `E2_system` across contexts, with its denominator | if the two contexts disagree on **either** certificate rate at the §13 decision level, the cycle verdict is **`conditioning_sensitive`** and **nothing is concluded about either certificate** (§13.4). **If they agree, BA4 licenses NOTHING additional**: the contexts are not independent (§7.3.2 condition 4), so agreement may not be reported as confirmation, corroboration or robustness evidence. **Unchanged from v2 in threshold and in consequence; only the wording of what it compares changed, because there is no longer a non-primary context to recompute in** |
...
| "a real out-of-set competitor **outside the stored candidate set** can beat `lp_best`" | **NO — this is the gap** | It is now a **reported quantity only**, in both contexts. No control demonstrates it and none can before freeze (§0.4). §21 item 9 records it as an accepted limitation rather than absorbing it |
...
cross-context difference interval is preregistered — the cross-context comparison is a **branch-level
agreement check** (§13.4), not an estimated contrast.
...
**NEW in v2.1 — the multiplicity exposure created by two co-primary contexts, and why no correction
is applied.** Reporting four one-sided bounds instead of two would, under a naive "report whichever
fires" reading, be two chances for each branch and an inflated error rate. **That reading is
forbidden by construction, not by a correction:**

- A branch is a **cycle-level** conclusion only if it fires in **both** contexts (§13.4). A branch
  that fires in one context and not the other **is** cross-context disagreement, which yields
  **`conditioning_sensitive`** and **nothing concluded about either certificate**.
- The cycle-level rule is therefore itself an **intersection–union test across contexts**: a
  conjunction of one-sided conditions, each at level 0.05, attaining level 0.05 with **no
  correction** and being **strictly harder** to fire than either context alone.
- **No threshold is moved to achieve this**; the conjunction is exactly BA4's frozen consequence
  from v2, applied at the cycle level now that both contexts are co-primary.
- What each context's own interval reports remains a per-context statement, labelled as such, with
  its one-sided 0.05 stated beside it. **It may not be promoted to a cycle-level conclusion on its
  own**, and §7.3.2 condition 4 separately forbids reading the second context as confirmation.
...
- **NEW in v2.1** — requiring a branch to fire in **both** co-primary contexts (§13.4) makes a
  cycle-level conclusion **strictly harder** to reach than v2's single-primary rule. The
  `p̂ ≥ 48 / 80 = 0.600` floor is a **necessary** condition in each context, not a sufficient one for
  the cycle. **This is a loss of power taken deliberately and disclosed in advance**, so that a
  `conditioning_sensitive` outcome is not later read as a failure of the experiment. It is the price
  of not privileging an already-observed context, and **no threshold was lowered anywhere to offset
  it**.
...
|---|---|---|
| `LB(P1) > 0.5` in **both** contexts | **YES, and decisive** | it refutes the E2-direction reading of the corpus with an interval, and in `C_gen` it fires `H0003` on evidence for the first time. This is the reason the cycle runs despite the E2 asymmetry |
...
| `conditioning_sensitive` | **YES** | and it is a finding, not a failure (§13.4) |
...
1. **If the two contexts fire the same §13.2 branch**, that branch is the cycle verdict — **with each
   context's own licensed language attached** (§8.7), which for the P1 branch differs between them.
   **Agreement licenses nothing beyond the per-context verdicts themselves.** The frozen sentence of
   §7.3.2 condition 4 is required wherever agreement is reported, and §1.4 item 9 forbids reading it
   as confirmation, corroboration, replication or robustness evidence.
2. **If the two contexts fire different branches at the decision level**, the cycle verdict is
   **`conditioning_sensitive`** and **nothing is concluded about either certificate**. Both
   per-context reads are still reported in full, with their intervals and their audits — the
   disagreement **is** the finding, and it bears directly on the question this cycle asks about the
   generator: whether the E2 / E3 reading depends on the conditioning the model is given.
3. **If one context is `undecidable` and the other is not**, the cycle verdict is
   **`conditioning_sensitive (one context undecidable)`**. The surviving read is reported in full with
   its language; it **may not** be promoted to a cycle-level conclusion on its own, because the
   §12.2 conjunction that keeps the error rate at 0.05 requires both.
4. **If NC-CONDITION failed**, BA4 is `uninterpretable` and the cycle verdict is
   **`conditioning_sensitive`** regardless of whether the branches agree — unchanged from v2's BA4
   consequence.
5. **Anticipated-difference disclosure, required.** Where a cross-context difference is traceable to a
   **known instrument asymmetry** rather than to the conditioning — specifically BA3 / Gate 3b, which
   is met 80 / 80 in `C_raw` and rests on unverified orbit members in `C_gen` (F-j), and the Gate-4
   reconstruction shortfall, which is a `C_gen`-only quantization effect (F-k) — the report must say
   so explicitly and must **not** present the difference as evidence that the certificate rate depends
   on the conditioning. Disagreement is informative only where its source is not already known to be
   an instrument artefact.

### 13.5 Every branch is informative — restated honestly

- `LB(P1) > 0.5` in both contexts fires `H0003` on evidence for the first time, on the strength of the
  `C_gen` read; the `C_raw` read contributes the downgraded sentence only.
...
- **Preregistered reduced designs** (invoked at Gate 5 or Gate 2a *before* starting, never midway):
  - **RD1** single context, **AMENDED in v2.1**: with both contexts co-primary there is no "primary
    ...
    Certificate-S language (§7.3.2 condition 1) — and reports `C_raw` as `not_measurable`. BA4 is then
    `not_measurable`, §13.4's cross-context conjunction cannot be evaluated, **no cycle-level verdict
    is formed**, the surviving read is reported as a **per-context** result only, and **a supported E2
    is blocked** under RD1 as it is everywhere else. **RD1 is not expected to be invoked**: §14
...
**NEW in v2.1 — the second context is NOT a replication.** `C_gen` and `C_raw` share the model, the
ground truths and the candidate set, so computing both satisfies **none** of the four independent
factors above. **`conditioning_sensitive` additionally fires this gate**, and so does a cycle verdict
that rests on agreement between the two contexts, precisely because that agreement is not independent
support (§7.3.2 condition 4). **C0002 contains no internal replication of its primary endpoint**
(§21 item 11).
...
12. **NEW in v2.1 — "A cycle-level verdict from a single context."** Not available by construction.
    §13.4 requires the two contexts to agree before any cycle-level branch fires, so a result that
    fires in one context only yields `conditioning_sensitive`. This is a deliberate loss of power
    (§12.4), taken so that the error rate across four reported bounds is controlled by a conjunction
    rather than by a correction, **and so that an already-observed context can never carry a
    conclusion alone.**
...
own as evidence for E2**; **§13.4's cycle-level rule that a branch fires only if both contexts fire
it, disagreement being `conditioning_sensitive`**; `H0010`'s three predicates, its **frozen 80-system /
...
`contexts_are_not_independent: true`**; and the reporting sentences of §8.7.
...
disposition a threshold carries** and **which context may say what** — never a number. §25 records
...
**And specifically, added in v2.1**: if the two contexts disagree, **`conditioning_sensitive` is the
result**, and the context that happened to fire the preferred branch may not be promoted to primary,
reported alone, or described as the "matched" or "correct" one. **If they agree, that agreement may
