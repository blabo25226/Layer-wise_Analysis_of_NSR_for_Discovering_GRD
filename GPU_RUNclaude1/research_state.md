...
**2026-09-10、利用者が確認: これは利用者による意図的な変更であり、伝達が漏れていたもの。**
誤設定でも障害でもない。したがって以下のアカウント種別は、当面のキャンペーンの
**恒常的な前提**として扱う。以前の前提（Claude Max プラン）は失効した。
ただし「組織が Claude Code のサブスクリプション利用を無効化していること」自体が
意図的な設定かは**別問題であり `unverified`**（下記「発生した障害」を参照）。
...
この前提は失効した。** 実態は組織のチームシートである。同ファイルの当該記述は 2026-09-10 に訂正した。
併せてセッション横断で参照される memory
（`claude-account-is-team-seat`）にも記録した — コールドセッションが
`research_state.md` を読む前にルーティング判断を誤らないようにするため。
...
> **訂正（2026-09-12、C0002 再現性監査）。** supervisor は当初「`sympy.core.random.seed(k)` を
> ワーカーごとに設定すれば閉じられる」と述べたが、**これは不十分である**。
> `rng` はインタプリタごとに 1 個のモジュール水準生成器であり、ワーカー開始時のシードは
> `t = 0` の状態を固定するにすぎない。ある比較時点の状態は、**そのワーカーがたまたま受け取った
> 先行の RNG 消費呼び出し**によって進んでおり、可用性スケジューリングのプールでは負荷依存になる。
> 監査は pinned 1.13.1 で実証した — `seed(123)` の直後に `.equals` を 1 回呼ぶと、
> `seed(123)` 単独とは異なる状態が残る。
> **有効な対策は入力のハッシュを鍵とする item ごとの決定論的再シードである。**
> 併せて `multiprocessing` の start method（`fork` は継承、`spawn` は継承しない）が
> 未固定であり、シードとファイアウォールガードの継承の**両方**がこれに黙って依存している。
