...
| `odeformer/` | [ODEFormer](https://github.com/sdascoli/odeformer) の `GitHubSourceCode/ODEFormer` から、出力埋め込みnotebookを除いてコピー | `sys.path` に載せて `odeformer` を import。公式 `setup.py` の `sympy==1.11.1` pinがあるため `pip install -e` は使わない |
...
- ODEFormer: 調査用snapshot `GitHubSourceCode/ODEFormer` からpackage / parsers / ODEBench / LICENSEをコピーした。`ODEFormer_demo.ipynb` と `scripts/tests.ipynb` は実行に不要な出力埋め込みnotebookのため除外し、調査用treeに残す。upstream `.git` は無く、公式commitはPhase 0のtree fingerprintで固定する。コピーしたODEFormerソース自体は変更していない。parser CLI defaultは論文checkpointと異なる場合があるため、architectureの根拠にしない。

## 重要な区別

- `GitHubSourceCode/NSRS`・`GitHubSourceCode/TPSR`・`GitHubSourceCode/ND2`・`GitHubSourceCode/ODEFormer` … 調査用の完全クローン。**実行時に import しない**
...
- `assets/odeformer/` … ODEFormer checkpoint（weightsはgitignore）
