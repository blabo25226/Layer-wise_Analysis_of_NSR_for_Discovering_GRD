...
5. Gather literature/repository evidence (Gemini-first compression/breadth via broker when needed; Cursor reconnaissance for repo structure).
...
11. Obtain independent adversarial review (Claude Opus 5.5; never skipped for capacity).
12. Replicate when required.
13. Write report (Gemini first draft → Claude polish → GPT-6 Sol PI final claims), manifest and checksums.
