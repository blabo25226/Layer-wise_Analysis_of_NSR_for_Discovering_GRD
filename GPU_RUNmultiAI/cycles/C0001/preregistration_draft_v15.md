# C0001 事前登録 v15 — メトリック同定可能性監査（自己完結改訂）
...
v14 targeted independent review の R14-1 から R14-5 を閉じる。
...
| 2 | Guard bootstrap | `guard_bootstrap.py` 経由で親/child が §11 順序を遵守；package/runtime import が install より前に発生しない；G4 前に side channel merge |
...
**post-freeze 必須モジュール `scripts/phases/guard_bootstrap.py`**（implementation は freeze 後・**G_contract** 合格後；本ファイルは §13.2 inventory に含むが **現時点では未実装可**）:

| 要件 | 凍結値 |
|---|---|
| 依存 | **stdlib のみ**（`os`, `sys`, `pathlib` 等） |
...
| matcher | `output_root_abs` と §11 deny 規則から `SealedPathGuard` matcher を構築 |
| install API | `install_guard_from_entry(entry_script_path: str) -> None`（guard を construct **かつ** install） |
...
2. `install_guard_from_entry(__file__)` — `output_root_abs` 構築と `SealedPathGuard.install()` を **単一呼び出し**で完了
3. 以降のみ audit runtime、corpus、plan、output artifacts、ODEFormer、task code を import/実行

**child `simplifier_worker.py` 起動順序（凍結）**:

1. `install_guard_from_entry(__file__)`
2. 以降のみ ODEFormer / simplifier / task code

**G_contract import-order acceptance**（post-freeze）:
テストは `gpu_runmultiai` package または `experiment_runtime` の import が `install_guard_from_entry` **より前に発生しない**ことを検証する。違反は `GuardBootstrapViolation`。
...
|---|---|---|
| parent audit (`run_audit`) | `install_guard_from_entry(__file__)` を **いかなる counted primitive / corpus 処理より前** | guard なしで `open` / `os.stat` 等 |
| simplifier child (`simplifier_worker.py`) | **ODEFormer / simplifier import より前** に `install_guard_from_entry(__file__)` | child 内 task code が guard なしで実行 |
...
- `scripts/phases/guard_bootstrap.py`（post-freeze 実装；G_contract 合格前は存在しなくても inventory 要件として列挙）
...
python scripts/phases/gpu_runmultiai_c0001_metric_audit.py \
  --audit-id c0001_metric_identifiability_audit_v15 \
  --output-dir GPU_RUNmultiAI/cycles/C0001/runs/c0001_metric_identifiability_audit_v15 \
...
- 本 v15 は **未凍結**
- 本 v15 の `audit_id` は **`c0001_metric_identifiability_audit_v15`** のみ
- 歴史 preregistration ファイル（v9–v14）は **編集しない**
