"""Stdlib-only sealed-path guard bootstrap (preregistration v16 §11)."""
...
class GuardBootstrapViolation(RuntimeError):
    """Guard bootstrap contract violation requiring global abort."""
...
def _resolve_repo_relative(path: object, repo_root: str) -> str:
...
def _path_strings(path: object, repo_root: str) -> tuple[str, str] | None:
    if _is_fd(path):
        return None
    fspath = _resolve_repo_relative(path, repo_root)
...
class BootstrapGuardHandle:
    ...
    def _check(self, operation: str, path: object) -> None:
        if self._in_internal:
            return
        resolved = _path_strings(path, self.repo_root)
        if resolved is None:
            return
        norm, real = resolved
...
_INSTALLED_GUARD: BootstrapGuardHandle | None = None


def get_installed_guard() -> BootstrapGuardHandle:
    if _INSTALLED_GUARD is None:
        raise GuardBootstrapViolation("guard bootstrap not installed")
    return _INSTALLED_GUARD


def install_guard_from_entry(entry_script_path: str) -> BootstrapGuardHandle:
    global _INSTALLED_GUARD
    if _INSTALLED_GUARD is not None:
        raise GuardBootstrapViolation("guard bootstrap singleton already installed")
    entry = Path(entry_script_path).resolve()
    repo_root = str(entry.parents[2])
    output_root_abs = os.path.join(repo_root, "results", "runs")
    handle = BootstrapGuardHandle(repo_root=repo_root, output_root_abs=output_root_abs)
    handle.install()
